...
import {
  ...
  FileSearch,
  List,
  MonitorSmartphone, // Icon for admin dashboard
} from "lucide-react";
...
type LoadingType =
  ...
  | "wishlist"
  | "cart"
  | "admin-dashboard" // New type for admin dashboard
  | "default";
...
export default function Loading({
  ...
  const loaderConfig = {
    ...
    "admin-dashboard": {
      icon: <MonitorSmartphone className="text-indigo-500" size={100} />, // Icon for admin dashboard
      defaultMessage: "Loading Admin Dashboard...",
  ...
  return (
    ...
    >
      ...
      >
        {[...Array(3)].map((_, index) => (
          <div key={index} className="w-3 h-3 bg-gray-500 rounded-full" />
        ))}
      </motion.div>
    </div>
  );
